...
(function($){
	$.fn.extend({
		slideList:function(options){
			...
			var moveLeft = moveWidth-eleWidth*options.scrollCount;
			var vLeft,vCount,timer = null;

			var page = 1;
			var pageCount = Math.floor(eleLength/options.scrollCount);
			// var refull = eleLength%options.scrollCount;

			slideObj.css({width:moveWidth});
			if(eleLength>options.scrollCount){
			...
			var mouseCtrl = {
				rightCtrl:{
					mouseDown:function(){
						...
						vCount >= eleLength-options.scrollCount ? rightCtrlObj.removeClass(options.rightCtrlClass.substring(1)+"On") : "";
						if(-vLeft >= moveLeft){
							clearInterval(timer);
					...
					mouseUp:function(){
						...
						vCount = Math.abs(Math.floor(vLeft/eleWidth));
						vCount >= eleLength-options.scrollCount ? rightCtrlObj.removeClass(options.rightCtrlClass.substring(1)+"On") : leftCtrlObj.addClass(options.leftCtrlClass.substring(1)+"On");
						if(-vLeft >= moveLeft){
							slideObj.css({left:-moveLeft});
						}else{
				...
				leftCtrl:{
					mouseDown:function(){
						...
						slideObj.css({left:vLeft - options.space});
						vCount = Math.abs(Math.floor(vLeft/eleWidth));
						vCount <= eleLength-options.scrollCount ? rightCtrlObj.addClass(options.rightCtrlClass.substring(1)+"On") : "";
				...
				rightCtrlObj.click(function(){
					if(!slideObj.is(":animated")){
						if(page >= pageCount){
							...
							slideObj.animate({left:slideObj.position().left-eleWidth*options.scrollCount},options.aniSpeed);
							page++;
